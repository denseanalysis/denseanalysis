...
opt.IsOctave = exist('OCTAVE_VERSION', 'var');
rootisarray=0;
rootlevel=1;
...
for e=1:len
  ...
  if(~isempty(names))
    for i=1:numel(names)
        txt=sprintf('%s%s',txt,obj2json(names{i},item(e).(names{i}), ...
             level+1+(len>1),varargin{:}));
        if(i<numel(names)); txt=sprintf('%s%s',txt,','); end
...
if(numel(size(item))>2 || issparse(item) || ~isreal(item) || ...
...
if(isunpack)
    ...
    if(~isoct)
        ...
        str0=name;
        pos0=[0 pend(:)' numel(name)];
        newname='';
        for i=1:numel(pos)
            newname=cat(2, newname, ...
                           str0(pos0(i)+1:pos(i)-1), ...
                           char(hex2dec(str0(pos(i)+3:pend(i)-1))));
        end
        if(pos(end)~=numel(name))
